...
@Component({
  selector: 'app-edicao-itens',
  templateUrl: './edicao-itens.component.html',
  styleUrls: ['./edicao-itens.component.css']
})
export class EdicaoItensComponent implements OnInit {
  produtos: any[] = [];
  ...
  editarProduto() {
    if (this.produtoSelecionado) {
      this.service
        .putProduto(this.produtoSelecionado.id, this.produtoSelecionado)
        .subscribe(
          (res) => {
      ...
      console.warn('Nenhum produto selecionado para edição.');
    }
  }

  salvarAlteracoes(produto: Produto) {
    const confirmacao = window.confirm('Deseja realmente salvar as alterações?');

    if (confirmacao) {
      this.service.putProduto(`${produto.rota}/${produto.id}`, produto).subscribe(
        (res) => {
          console.log('Produto atualizado com sucesso:', res);
        },
        (err) => {
